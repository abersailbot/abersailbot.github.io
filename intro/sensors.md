# Parts needed
 * Raspberry Pi 
 * USB Serial GPS on Raspberry Pi
 * USB RS232 converter
 * RS232 Cable
 * USB serial converter (TTL)
 * CMPS12, HMC6352 or 6343 compass
 * rotary wind sensor
 * ultrasonic wind sensor + 12V PSU
 * Dewi
 * oscilliscope
 ...
 * Login to the boat
 * Run `gpspipe -r` to show the raw NMEA data
 * Run the command `cgps` to see interpreted data
 * How accurate is your GPS position (latitude and longitude)? Try entering the coordinates into Google Maps.
 * What happens if you move the receiver further from the window?
 * How fast does the GPS claim you are moving? Are you currently moving?
 * How accurate is the GPS altitude? Check streetmap.co.uk for Ordnance Survey maps.


# Compass

Senses the earth's magnetic field to find the direction to north. Dewi's compass is a Honeywell HMC6343. It costs about £100 but works well. The HMC5883 and 6352 are cheaper and simpler versions.
The XPBs use the CMPS11, which is much cheaper (about £30) but not quite as good.
...
Aberystwth about -2 degrees, East coast USA -12, West coast +13
...
Login to Dewi and read the compass
1. Connect to Dewi wifi network
2. ssh pi@192.168.40.1
3. curl http://127.0.0.1:2000/boat
   or
   screen /dev/arduino 115200 
   press c to read the compass

Or you can probably visit http://192.168.40.1:2000/boat from your laptop's web browser.

### LMS303d compass

This is a really cheap compass, gyro and accelerometer on one board, but you have to integrate the data together in software to derive a heading. 
The software from [SeedStudio](https://www.seeedstudio.com/blog/2019/12/17/lsm303-accelerometer-arduino-raspberry-pi-guide-to-get-started/) will attempt to generate angles, but it never seems to give sensible readings.
Run it by launching: /home/pi/GrovePi/Software/Python/grove_6axis_acc_compass on the Pi with the LMS303d.
...
### Read the wind sensor on Dewi

Via boatd:

curl localhost:2000/wind 
